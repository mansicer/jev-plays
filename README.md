...
Craftax-Classic, 22 achievements, 3 seeds, 2000-step horizon. Per-run logs and the script that recomputes every number: [`results/`](results/RESULTS.md). Every episode eventually ends in death; the differences of ≤2 achievements are within seed noise. For scale, PPO-RNN trained for 1B environment steps reaches ≥90 % of the maximum reward (≈20/22) on 10 000-step episodes, and humans about 65 %.
...
Python 3.12 and [uv](https://docs.astral.sh/uv/); a TypeSafe API key for jev, and an OpenAI-compatible endpoint for the planner or the LLM agent.
...
Checks: `uv run pytest` runs the no-key tests (wrapper smoke test on both game variants, and a logged episode replayed step by step must reproduce its achievements and death); `uv run pytest -m slow` audits the harness's legality rules against the game's own step function; `uv run pytest -m jev` makes real Jev calls.
